          ...
          <ul class="nav navbar-nav float-right">
            <li class="dropdown dropdown-user nav-item">
              ...
              <div class="dropdown-menu dropdown-menu-right">
                ...
                <a class="dropdown-item" href="{{ route('signoutdoctors') }}" onclick="event.preventDefault(); document.getElementById('logout-form').submit();"> تسجيل الخروج </a>
                <form id="logout-form" action="{{ route('signoutdoctors') }}" method="POST" class="d-none">
                  @csrf
                </form>
            ...
            <li class="dropdown dropdown-notification nav-item">
              <a class="nav-link nav-link-label" href="#" data-toggle="dropdown"><i class="ficon ft-bell"></i>
                <span class="badge badge-pill badge-default badge-danger badge-default badge-up badge-glow">{{$not_doctor_count}}</span>
              </a>
              <ul class="dropdown-menu dropdown-menu-media dropdown-menu-right">
                <li class="dropdown-menu-header">
                  ...
                  <span class="notification-tag badge badge-default badge-danger float-right m-0">{{$not_doctor_count}} New</span>
                </li>
                <li class="scrollable-container media-list w-100">
                  
                  @foreach ($patient_notifications_doctor as $_patient) 
                    @foreach ($_patient->unreadnotifications as $_patients)
                    <!--{{url('view/notification/'.$_patients->id)}}-->
                       <a href="#">
                        <div class="media">
                          <div class="media-left align-self-center">
                            @if($_patient->photo !=null)
                              <img class="avatar-img rounded-circle" alt="User Image" src="{{asset('assets_admin/img/patient/'.$_patient->photo) }}"  width="50px">
                            @else                                      
                          ...
                          <div class="media-body">
                            <h6 class="media-heading"> حجز جديد من {{$_patients->data['name']}}</h6>
                            <p class="notification-text font-small-3 text-muted">{{$_patients->data['message']}}:</p>
                            <small>
                              <time class="media-meta text-muted" datetime="2015-06-11T18:29:20+08:00">{{$_patients->data['orderId']}}</time>
                            </small>
                          </div>
                        </div>
                      </a>
