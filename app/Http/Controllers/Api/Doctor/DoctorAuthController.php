...
class DoctorAuthController extends Controller
{
    ...
    public function register(Request $request)
    {
        ...
        if($checkemail){
            ...
            $work_days->doctorId  = $add->id;
            $work_days->save();
            
            $day= new Day;
            $day->work_dayId  = $work_days->id;
            $day->save();
            
            $time= new Time;
            $time->work_dayId  = $work_days->id;
            $time->save();
            
            $doctor_license= new Doctor_license;
            $doctor_license->doctorId  = $add->id;
            $doctor_license->save();
   ...
   public function forgetPassword(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            try {
                ...
                if($doctorss==null){
                    return $this -> returnError('404','البريد الإلكتروني غير موجود');
                }else{
                    ...
                    $doctorss->save();
                    $details = [
                        'title' => 'صحتي وجمالي - تغيير كلمة المرور',
                        'body' => 'انسخ كود كلمة المرور للدخول لصحتي وجمالي: ' ." " . $gene . " "
                    ];
