...
{  
    ...
    {    
        ...
        foreach ($vendors as $item) {
            $item->photo="http://beautiheath.com/assets_admin/img/vendors/".$item->photo;
        }    
    ...
    {    
        // dd('jj');
        $vendors = Product::selection()->where('vendorId',$request->vendorId)->get();
        foreach ($vendors as $item) {
            // $name = json_decode($item->name);
            // $description = json_decode($item->description);
            // if($request->lang=="ar"){
            //      $item->name=$name->ar;
            //      $item->description=$description->ar;
            // }else{
            //     $item->name=$name->en;
            //     $item->description=$description->en;
            // } 
            $item->cover_img="http://beautiheath.com/assets_admin/img/products/".$item->cover_img;
    ...
    public function createOrder(Request $request)
    {
            ...
            if($request->type ==0){
                ...
                // if category type =1 it is buy product
            ...
            $patient->notify(new \App\Notifications\PatientNewNotification(
                        $patient->first_name,
                        $request->doctorId,
                        $message,
                        $patient->photo,
            ...
            $patients= Patient::where('id',$add->patientId)->first();
            // dd($patients);
        ...
        //////////////////////////
            ...
            foreach($doctor_blocked as $item)
            {
                ...
                $doctor = Doctor::where('id',$item->id)->first();
                    ...
                    $doctor->personality_photo="http://beautiheath.com/assets_admin/img/doctors/photo/".$doctor->personality_photo;
                    $item->doctor = $doctor;
