...
class VendorController extends Controller
{
    ...
    public function index()
    {
        ...
        $cities=City::all();
        // dd('ddddd');
        return view('admin.vendors.all',compact('vendors','specialities','countries','cities'));
    }

    public function destroy(Request $request )
    {

        $delete = Vendor::findOrFail($request->id);
        $delete->delete();
    ...
    public function profile($vendor)
    {
        $vendors = Vendor::findOrFail($vendor);
        $products=Product::where("vendorId" , $vendors->id)->get();
        $orders=Order::where('vendorId',$vendor)->get();
        foreach($orders as $item){
            ...
            $doctor = Vendor::where('id',$item->vendorId)->first();
            if($doctor){
            ...
            foreach ($order_product as $order_product_item) {  
                $productss[]=Product::where("id" , $order_product_item->orderProductId)->first();
            }
            foreach ($productss as $product_item) {
                $product_item->image="https://findfamily.net/care/assets_admin/img/products/".$product_item->image;
            }
            $item->products=$productss;
        }
        // dd($orders);
        return view('admin.vendors.vendor-profile',compact('orders','vendor','vendors','products'));
    }
    ...
    public function vendornotifaction()
    {
        $vendors= Vendor::all();
        return view('admin.vendors.notifaction',compact('vendors'));
    }

    public function vendor_Notifaction(Request $request)
    {
        // dd($request->doctorId);
        // dd($request->message);
        $length = count($request->vendorId);
        if($length > 0)
        {
            for($i=0; $i<$length; $i++)
            {  
                $vendor = Vendor::find($request->vendorId[$i]);
