...
class UserController extends Controller
{
    ...
    public function vendorActivation($token){
	   // dd('bvnvb');
	    $check = DB::table('user_activations')->where('token',$token)->first();
	  	if(!is_null($check)){
		    $user = Vendor::find($check->id_user);
		    if ($user->is_activated ==1){
		      return redirect()->to('/user-login')->with('message',"حسابكم مفعل ");
		    }
		    $user->update(['is_activated' => 1]);
		    DB::table('user_activations')->where('token',$token)->delete();
		    return redirect()->to('/user-login')->with('message',"تم تفعيل حسابك");
	  	}
	    return redirect()->to('/user-login')->with('errorss',"الرمز الخاص بك غير صالح");
	}
